...
NTSTATUS CUsbDkFilterDeviceInit::Configure(ULONG InstanceNumber)
{
    ...
    // 3.注册链接名称(每创建一个对象序号++)
    // 4.SetPowerCallbacks
    // 5.SetIoInCallerContextCallback
    // 6.SetFileEventCallbacks
    // 7.SetPreprocessCallback  IRP_MJ_PNP
    PAGED_CODE();
    ...
    //设置成过滤器 WdfFdoInitSetFilter
    SetFilter();

    CString DeviceName;
    //注册链接\\Device\\UsbDkFilter0 \\Device\\UsbDkFilter1
...
NTSTATUS CUsbDkFilterDevice::DefineStrategy()
{
    ...
    //CUsbDkHubFilterStrategy.Create
...
{   //这里是UsbDkEvtDeviceAdd传进来的
    //初始化m_DeviceInit = DeviceInit; PWDFDEVICE_INIT
    CUsbDkFilterDeviceInit DeviceInit(DevInit);

    TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_FILTERDEVICE, "%!FUNC! Entry");
    // CInstanceCounter<CUsbDkFilterDevice> m_InstanceNumber; 构造函数自动 InterlockedIncrement 但一直等于0 -> 1 
    // 1.初始化设备队列上下文
    // 2.设置成过滤器
    // 3.注册链接名称(每创建一个对象序号++)
    // 4.SetPowerCallbacks
